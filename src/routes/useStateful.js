...
const useStateful = () => {
  ...
  const getTodo = (id) => {
    //este id que viene por parametro proviene de el hook "useParams" desde EditTodoPage y como es dinamico (para eso es useParams ejem. /:slot, /:id) va a enviar el id como parametro del todo que seleccionemos en la UI.
  ...
  const editTodo = (id, newText) => {
    //este id que viene por parametro proviene de el hook "useParams"
    const newTodos = [...todos];
    const todoIndex = todos.findIndex((item) => item.id === id);
  ...
  const deleteTodo = (id) => {
    const newTodos = [...todos];
    const todoIndex = newTodos.findIndex((copyItem) => copyItem.id === id);
